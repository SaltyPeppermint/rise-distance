...
impl<C: Counter, L: Label> TermCount<'_, C, L> {
    ...
    #[must_use]
    pub fn sample_with_overlap<R: Rng + SeedableRng>(
        &self,
        eclass_id: EClassId,
        ref_tree: &TreeNode<L>,
        target_size: usize,
        ...
        self.fill_holes_sampling(remaining, rng, partial)
    }
...
mod tests {
    ...
    fn sample_with_overlap_exact_match() {
        ...
        let result = tc
            .sample_with_overlap(EClassId::new(0), &ref_tree, 2, &mut rng)
            .unwrap();
    ...
    fn sample_with_overlap_with_holes() {
        ...
        let result = tc
            .sample_with_overlap(EClassId::new(0), &ref_tree, 3, &mut rng)
            .unwrap();
    ...
    fn sample_with_overlap_budget_too_small() {
        ...
        assert!(
            tc.sample_with_overlap(EClassId::new(0), &ref_tree, 1, &mut rng)
                .is_none()
        );
    }

    #[test]
    fn sample_with_overlap_no_match_falls_back() {
        // Class 0: leaf "a", leaf "b"
        // ref_tree: "z" — no match, falls back to plain sampling
        let graph = EGraph::new(
            cfv(vec![EClass::new(
                vec![ENode::leaf("a".to_owned()), ENode::leaf("b".to_owned())],
                dummy_ty(),
        ...
        let result = tc
            .sample_with_overlap(EClassId::new(0), &ref_tree, 1, &mut rng)
            .unwrap();
        assert_eq!(result.size(false), 1);
