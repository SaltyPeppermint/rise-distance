...
fn is_all_same_memory_or_zero(
    vars: &[String],
) -> impl Fn(&mut EGraph<VecLang, ()>, Id, &Subst) -> bool + use<> {
    ...
    move |egraph, _, subst| {
        ...
        let non_zero_unique = vars
            ...
            .filter(|v| seen.insert(egraph.find(subst[**v])))
            .count();
        non_zero_unique < 2
    }
}
...
pub fn build_binop_rule(op_str: &str, vec_str: &str) -> Rewrite<VecLang, ()> {
    let searcher: Pattern<VecLang> = vec_fold_op(op_str, "a", "b").parse().unwrap();

    let applier: Pattern<VecLang> =
        format!("({} {} {})", vec_str, vec_with_var("a"), vec_with_var("b"))
            .parse()
            .unwrap();
...
pub fn build_unop_rule(op_str: &str, vec_str: &str) -> Rewrite<VecLang, ()> {
    let searcher: Pattern<VecLang> = vec_map_op(&op_str.to_owned(), &"a".to_owned())
        .parse()
        .unwrap();
    let applier: Pattern<VecLang> = format!("({} {})", vec_str, vec_with_var("a"))
        .parse()
        .unwrap();
...
pub fn build_litvec_rule() -> Rewrite<VecLang, ()> {
    let mem_vars = ids_with_prefix("a", VECTOR_WIDTH);
    let gets: Vec<String> = (0..VECTOR_WIDTH)
        .map(|i| format!("(Get {} ?{}{})", mem_vars[i], "i", i))
        .collect();
