...
fn eval(
    egraph: &EGraph<Lambda, LambdaAnalysis>,
    enode: &Lambda,
...
impl Analysis<Lambda> for LambdaAnalysis {
    ...
    fn make(egraph: &mut EGraph<Lambda, LambdaAnalysis>, enode: &Lambda, _id: Id) -> Data {
    ...
    fn modify(egraph: &mut EGraph<Lambda, LambdaAnalysis>, id: Id) {
...
fn is_not_same_var(
    v1: Var,
    v2: Var,
) -> impl Fn(&mut EGraph<Lambda, LambdaAnalysis>, Id, &Subst) -> bool {
...
impl Applier<Lambda, LambdaAnalysis> for CaptureAvoid {
    fn apply_one(
        &self,
        egraph: &mut EGraph<Lambda, LambdaAnalysis>,
        eclass: Id,
        subst: &Subst,
...
mod tests {
    ...
    #[test]
    fn nested() {
        let expr: RecExpr<Lambda> = "(app (lam x (var x)) 0)".parse().unwrap();
        assert_eq!(expr.to_string(), "(app (lam x (var x)) 0)");
    }
    ...
    fn fromstr_roundtrip() {
        ...
        for (s, expected) in cases {
            let parsed: RecExpr<Lambda> = s.parse().unwrap();
            assert_eq!(parsed[parsed.root()], *expected, "fromstr failed for {s:?}");
