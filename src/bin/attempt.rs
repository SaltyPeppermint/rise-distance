...
//! the same flag takes a plain s-expression and the run is the unguided
//! baseline. Prints the run's `Result<ReachedRun, GuideError>` as the `payload`
//! of a [`Measured`] envelope.
//!
...
#[command(
    ...
    after_help = "\
Reads nothing on stdin. With `--is-guide`, `--start-term` is one attempt's guide
as a JSON array of guide nodes; without it, `--start-term` is a plain
s-expression and this is the unguided baseline. Either way the result is printed
as a JSON object. `--goal-term`, `--language`, and the eqsat limits come from
argv. The driver runs the search loop, one process per attempt, so each
process's peak RSS covers exactly one eqsat run — the same unit as the unguided
baseline. Example:
  attempt --language math --goal-term '(+ x 0)' --start-term 'x' \\
