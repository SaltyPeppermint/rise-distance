...
use crate::{Graph, PartialChild, PartialTree, TypedTree, tree_node_to_partial};
...
pub fn match_ref_tree<L: Label>(
    ...
    let ty = TypedTree::from_eclass(graph, canonical_id);
...
mod tests {
    ...
    fn match_ref_tree_exact() {
        ...
        let ref_tree: TypedTree<String> = "(f a)".parse().unwrap();
    ...
    fn match_ref_tree_partial_hole() {
        ...
        let ref_tree: TypedTree<String> = "(f c)".parse().unwrap();
    ...
    fn match_ref_tree_no_match_at_root() {
        ...
        let ref_tree: TypedTree<String> = "b".parse().unwrap();
    ...
    fn prune_matched_class_reachable_from_hole_is_protected() {
        ...
        let ref_tree: TypedTree<String> = "(f a c)".parse().unwrap();
        let (pruned, pruned_ids) = prune_by_ref_tree(&graph, EClassId::new(0), &ref_tree);
