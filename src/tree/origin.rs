...
pub struct OriginTree<L: Label + Language> {
    label: L,
...
impl<L: Label + Language> OriginTree<L> {
    ...
    #[must_use]
    pub fn from_eclass<N: Analysis<L>>(graph: &EGraph<L, N>, id: Id) -> Option<Self> {
        let ty_id = graph[id].ty()?;
