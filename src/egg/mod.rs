...
// pub fn convert<L, N, LL>(egg_graph: &EGraph<L, N>, root: Id) -> EGraph<LL>
// where
//     L: Language,
//     N: Analysis<L>,
//     LL: Language + for<'a> From<&'a L>,
// {
...
where
    ...
    N: Analysis<L>,
    D: IterationData<L, N>,
    I: IntoIterator<Item = &'a OriginExpr<L>>,
{
    for guide in guides {
        let expr = origin::strip(guide.to_owned());
...
where
    ...
    N: Analysis<L>,
{
    let mut adder = |node: &OriginNode<L>, lang_node| {
        let new_id = graph.add_uncanonical(node.node);
        origin_to_new_ids
            .entry(node.origin())
            .or_default()
            .insert(new_id);
...
pub fn valididty_hook<L: Language + Display, N: Analysis<L> + Default>(
    expr: &RecExpr<L>,
...
pub struct TypeAnalysisWrapper<N> {
    inner: N,
    type_id: TypeId,
}

impl<L: Language, N: Analysis<L>> Analysis<L> for TypeAnalysisWrapper<N> {
    type Data = TypeId;
...
pub struct TypeId(pub Option<Id>);

impl TypeId {
    pub fn add_type<L: Language, N: Analysis<L>>(
        &self,
        graph: &EGraph<L, N>,
        expr: &mut RecExpr<OriginNode<L>>,
    ) -> Option<Id> {
        todo!()
    }

    pub fn size<L: Language, N: Analysis<L>>(&self, graph: &EGraph<L, N>) -> usize {
        todo!()
    }
}
